...
class AffiliatePlatformProductsModel:
    ...
    def get(self, affiliate_platform_product_id: str) -> Optional[AffiliatePlatformProduct]:
        """
        Retrieve an affiliate platform product's details based on its ID.
        ...
        """
        affiliate_platform_product = self.db.connection[self.collection].find_one({"_id": ObjectId(affiliate_platform_product_id)})
    ...
    def get_all(self) -> List[AffiliatePlatformProduct]:
        """
        Retrieve all affiliate platform products from the database.

        :return: A list of AffiliatePlatformProduct objects representing all the items in the database.
        :rtype: list[AffiliatePlatformProduct]
        """
        affiliate_platform_products = [AffiliatePlatformProduct(**item) for item in self.db.connection[self.collection].find()]
        return affiliate_platform_products if affiliate_platform_products else []
